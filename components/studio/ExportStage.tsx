...
export default function ExportStage({
  ...
  async function downloadOne(i: number) {
    ...
    try {
      ...
      alert('카드 렌더에 실패했어요. 잠시 후 다시 시도해 주세요.');
    } finally {
      setBusy(null);
    }
  }
  ...
  async function downloadZip() {
    ...
    try {
      ...
      alert('ZIP 생성에 실패했어요. 잠시 후 다시 시도해 주세요.');
    } finally {
      setBusy(null);
    }
  }
  ...
  return (
    <div className="export">
      <div className="export__l">
        <span className="done"><span className="o">✓</span> Ready to post</span>
        <h2>피드에 올릴 준비가 됐어요</h2>
        <p>인스타그램 캐러셀로 어떻게 보일지 미리 확인하고, 카드와 캡션을 받아 그대로 올리세요.</p>

        <div className="phone">
          <div className="phone__bar"><span className="av" /><span className="h">{magazine.handle.replace('@', '')}</span><span className="more">⋯</span></div>
          <div className="carousel">
            <div className="carousel__track" style={{ transform: `translateX(${-cur * 100}%)` }}>
              {cards.map((c, i) => (
                <div className="cslide" key={i} style={{ position: 'relative' }}>
                  <CardFace card={c} magazine={magazine} ctx="slide" />
                </div>
              ))}
            </div>
            <span className="carousel__count">{cur + 1}/{n}</span>
            <button className="carousel__nav prev" onClick={() => slide(-1)}>‹</button>
            <button className="carousel__nav next" onClick={() => slide(1)}>›</button>
          </div>
          <div className="dots">
      ...
      <div className="export__r">
        <div className="zip">
          ...
          <button className="btn btn--lg" onClick={downloadZip} disabled={busy !== null}>
        ...
        <div className="panel">
          ...
          <div className="panel__b">
            <div className="dlrow">
              {cards.map((_, i) => (
                <button className="minib" key={i} onClick={() => downloadOne(i)} disabled={busy !== null}>
                  {busy === `png-${i}` ? '◌' : <>0{i + 1} ⤓</>}
                </button>
              ))}
            </div>
