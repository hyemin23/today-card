 ...
 *
 * IMAGE MODEL POLICY (see design-system/design.md §8): default provider is
 * Gemini's native image model — "Nano Banana" (NB1) = `gemini-2.5-flash-image`
 * (~$0.039/img) by default, or NB2 = gemini-3-pro-image-preview (~$0.134/img)
 * via IMAGE_MODEL. Higgsfield is an opt-in alternate provider (§8.6) — selected
 * per-request or via IMAGE_PROVIDER=higgsfield — never the silent default.
 */

import { loadImageRules, type ImageRules } from './imageRules';
import { generateImageViaHiggsfield } from './higgsfield';
...
export async function generateCardImage(
  ...
  const prompt = buildPrompt(title, category, style, rules, concept, customLook);
  const chosen = provider || (process.env.IMAGE_PROVIDER as ImageProvider | undefined) || 'gemini';

  return chosen === 'higgsfield' ? generateImageViaHiggsfield(prompt) : generateImageViaGemini(prompt, apiKey);
}
